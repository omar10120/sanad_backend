...
@section('title')
    {{ trans('main_trans.Edit_notification') }}
@endsection
@section('css')
    <!--Internal   Notify -->
...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{ trans('main_trans.Notifications') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('main_trans.Edit_notification') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">{{ trans('main_trans.Edit_notification') }}</h4>
                ...
                <div class="card-body">
                    <form action="{{ route('notifications.update', $notification->id) }}" method="post" autocomplete="off">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                                        ...
                                        <option value="general" {{ old('type', $notification->type) == 'general' ? 'selected' : '' }}>{{ trans('main_trans.General') }}</option>
                                        <option value="question_update" {{ old('type', $notification->type) == 'question_update' ? 'selected' : '' }}>{{ trans('main_trans.Question_update') }}</option>
                                        <option value="system" {{ old('type', $notification->type) == 'system' ? 'selected' : '' }}>{{ trans('main_trans.System') }}</option>
                                        <option value="announcement" {{ old('type', $notification->type) == 'announcement' ? 'selected' : '' }}>{{ trans('main_trans.Announcement') }}</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('body') is-invalid @enderror" id="body" name="body" rows="4" required>{{ old('body', $notification->body) }}</textarea>
                            @error('body')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('target_type') is-invalid @enderror" id="target_type" name="target_type" required>
                                        <option value="">{{ trans('main_trans.Select') }}</option>
                                        <option value="all" {{ old('target_type', $notification->target_type) == 'all' ? 'selected' : '' }}>{{ trans('main_trans.All_students') }}</option>
                                        @if(config('features.advanced_notifications'))
                                        <option value="type" {{ old('target_type', $notification->target_type) == 'type' ? 'selected' : '' }}>{{ trans('main_trans.By_type') }}</option>
                                        <option value="student" {{ old('target_type', $notification->target_type) == 'student' ? 'selected' : '' }}>{{ trans('main_trans.By_student') }}</option>
                                        @else
                        ...
                        <div class="form-group" id="target_types_div" style="display: none;">
                            ...
                            <select class="form-control select2 @error('target_ids') is-invalid @enderror" id="target_types" name="target_ids[]" multiple>
                                @foreach($types as $type)
                                    <option value="{{ $type->id }}" {{ in_array($type->id, old('target_ids', $notification->target_ids ?? [])) ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                            @error('target_ids')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-group" id="target_students_div" style="display: none;">
                            ...
                            <select class="form-control select2 @error('target_ids') is-invalid @enderror" id="target_students" name="target_ids[]" multiple>
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" {{ in_array($student->id, old('target_ids', $notification->target_ids ?? [])) ? 'selected' : '' }}>{{ $student->first_name }} {{ $student->last_name }}</option>
                                @endforeach
                            </select>
                            @error('target_ids')
                                <div class="invalid-feedback">{{ $message }}</div>
