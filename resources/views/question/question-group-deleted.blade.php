...
@section('title')
{{ trans('main_trans.Deleted_question_groups') }}
@endsection
@section('css')
...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ trans('main_trans.Study') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('main_trans.Deleted_question_groups') }}</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
                            ...
                            <div class="card-header pb-0 row">
                                <div class="col-12 col-sm-12 col-lg-2 col-xl-2">
                                    <div class="pull-right">
                                        @if(isset($lesson_selected))
                                            <a class="btn btn-primary btn-block" href="{{ route('lesson.questionGroup', $lesson_selected->id) }}"><i class="fas fa-arrow-right"></i>{{' ' . trans('main_trans.Back') }}</a>
                                        @else
                                            <a class="btn btn-primary btn-block" href="{{ route('lesson.index') }}"><i class="fas fa-arrow-right"></i>{{' ' . trans('main_trans.Back') }}</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="table-responsive hoverable-table">
                                    <table class="table table-hover" id="example1" data-page-length='100' style=" text-align: center;">
                                        ...
                                        <tr>
                                            ...
                                            <th class="wd-15p border-bottom-0">{{ trans('main_trans.Lesson') }}</th>
                                            <th class="wd-10p border-bottom-0">{{ trans('main_trans.Order') }}</th>
                                            <th class="wd-10p border-bottom-0">{{ trans('main_trans.Number_of_questions') }}</th>
                                            <th class="wd-10p border-bottom-0">{{ trans('main_trans.Actions') }}</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @foreach ($archivedQuestionGroups as $questionGroup)
                                            <tr>
                                                <td>{{ $questionGroup->id }}</td>
                                                <td>{{ $questionGroup->name }}</td>
                                                <td>{{ $questionGroup->lesson->title }}</td>
                                                ...
                                                <td>
                                                    @can('Question-restore-deleted')
                                                        <a class="modal-effect btn btn-info" data-effect="effect-flip-vertical"
                                                           data-id="{{ $questionGroup->id }}" data-name="{{ $questionGroup->name }}" data-toggle="modal"
...
<!-- End restore -->
                    ...
                    <div class="modal" id="modal3">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content modal-content-demo">
                                <div class="modal-header">
                                    <h6 class="modal-title">{{ trans('main_trans.Delete_question_group') }}</h6><button aria-label="Close" class="close" data-dismiss="modal"
                                                                                    type="button"><span aria-hidden="true">&times;</span></button>
                                </div>
                                <form action="{{route('archived-question-group.destroy','archived-question-group')}}" method="post">
                                    {{method_field('delete')}}
                                    {{csrf_field()}}
                                    <div class="modal-body">
