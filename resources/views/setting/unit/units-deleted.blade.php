...
@section('title')
    {{ trans('main_trans.Deleted_units') }} - {{ $teacher_selected->name }}
@endsection
@section('css')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('main_trans.Deleted_units') }} - {{ $teacher_selected->name }}</span>
            </div>
        </div>
    </div>
@endsection
@section('content')
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <a class="btn btn-primary btn-sm"
                       href="{{ route('teacher.unit', ['teacher' => $teacher_selected->id, 'subject_video' => $subject_video_selected->id]) }}">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">{{ trans('main_trans.Restore_unit') }}</h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <form action="{{ route('archived-unit.update') }}" method="post">
                    @csrf
                    @method('PATCH')
    ...
    <div class="modal" id="modal3">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">{{ trans('main_trans.Delete_unit_permanently') }}</h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <form action="{{ route('archived-unit.destroy') }}" method="post">
                    @csrf
                    @method('DELETE')
                    <div class="modal-body">
