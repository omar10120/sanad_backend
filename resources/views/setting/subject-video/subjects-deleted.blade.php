...
@section('title')
    {{ trans('main_trans.Deleted_course_subjects') }} - {{ $type_selected->name }}
@endsection
@section('css')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('main_trans.Deleted_course_subjects') }} - {{ $type_selected->name }}</span>
            </div>
        </div>
    </div>
@endsection
@section('content')
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header pb-0">
                    <a class="btn btn-primary btn-sm" href="{{ route('type.subject-video', $type_selected->id) }}">
                        ...
                        <table class="table table-hover" id="example1" data-page-length='100' style="text-align: center;">
                            ...
                            @foreach ($subjectVideos as $subjectVideo)
                                <tr>
                                    ...
                                    <td>{{ $subjectVideo->name }}</td>
                                    <td>{{ $subjectVideo->teachers_count }}</td>
                                    <td>
                                        @can('SubjectVideo-restore-deleted')
                                            <a class="modal-effect btn btn-info" data-effect="effect-flip-vertical"
                                               data-id="{{ $subjectVideo->id }}" data-name="{{ $subjectVideo->name }}" data-toggle="modal"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">{{ trans('main_trans.Restore_course_subject') }}</h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <form action="{{ route('archived-subject-video.update') }}" method="post">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <p>{{ trans('main_trans.Are_you_sure_to_restore') }}</p>
                        <input type="hidden" name="id" id="restore_id">
                        <input type="hidden" name="type_id" value="{{ $type_selected->id }}">
    ...
    <div class="modal" id="modal3">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">{{ trans('main_trans.Delete_course_subject_permanently') }}</h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <form action="{{ route('archived-subject-video.destroy') }}" method="post">
                    @csrf
                    @method('DELETE')
                    <div class="modal-body">
