...
class ApiSubjectVideoService
{
    ...
    public function getUnitsByTeacher(int $teacherId): Collection
    {
        return Unit::where('teacher_id', $teacherId)
            ...
            ->withCount(['lessonVideos' => fn ($query) => $query->where('is_active', true)])
            ->orderBy('order')
            ->get();
    }
    ...
    public function getUnitsByTeacherWithContent(int $teacherId): Collection
    {
        return Unit::where('teacher_id', $teacherId)
            ...
            ->withCount(['lessonVideos' => fn ($query) => $query->where('is_active', true)])
            ->with([
                'lessonVideos' => fn ($query) => $query->where('is_active', true)->orderBy('order'),
                // 'lessonVideos.youtubeLinks' => fn ($query) => $query->where('is_active', true)->orderBy('order'),
            ])
            ->orderBy('order')
            ->get();
    }
    ...
    {
        return Unit::where('is_active', true)->withCount('lessonVideos')->find($id);
    }

    public function getLessonVideosByUnit(int $unitId, bool $isLocked): Collection
    {
        $query = LessonVideo::where('unit_id', $unitId)->where('is_active', true)
